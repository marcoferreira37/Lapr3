...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for (V otherVert : mapVertices.keySet()) {
            MapVertex<V, E> mv = mapVertices.get(otherVert);
    ...
    @Override
    public Collection<Edge<V, E>> edgesOf(V vert) {
        if (!validVertex(vert))
            return null;
    ...
    @Override
    public V getSource(Edge<V, E> edge) {
        if (edge == null)
            return null;

        return edge.getVOrig();
    }

    @Override
    public V getTarget(Edge<V, E> edge) {
        if (edge == null)
            return null;
