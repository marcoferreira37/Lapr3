...
public class OperacaoMondaRepository {
    public OperacaoMondaRepository(){
    }

    public void mondaRegister(Date data, String operacao, int quantidade, String unidade, String designacao, int idCultura, int id) throws SQLException {
        Connection connection= DatabaseConnection.getInstance().getConnection();
        try(CallableStatement callableStatement=connection.prepareCall("{? = call fncUS12(?,?,?,?,?,?,?)}")){
